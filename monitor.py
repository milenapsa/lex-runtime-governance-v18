...
def cycle():
    ...
    checks["registry"]=status==200 and s.get("registered_unique")==30 and s.get("active_search")==17 and s.get("blocked")==3
    ...
    status,q=post("Lei 13.709/2018")
    checks["search"]=status==200 and q.get("integrity",{}).get("synthetic")==0 and "senado_legislacao" in q.get("sources_used",[])
    payload={"ts":now(),"service":"lex-runtime-monitor","version":VERSION,"status":"ok" if all(checks.values()) else "degraded","checks":checks,"trace_id":q.get("trace_id")}
